...
export default function ControlsPage() {
  ...
  const loadData = useCallback(() => {
    ...
    const params = new URLSearchParams({ filter: "daily", house_id: selectedHouse })
    fetch(`/api/sensor-readings?${params}`)
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data)) setReadings(data) })
      .catch(() => {})
  }, [selectedHouse])
  ...
  const avgTemp = tempHistory.length ? (tempHistory.reduce((a, b) => a + b, 0) / tempHistory.length).toFixed(1) : "0.0"
  ...
  const effectiveHeater = heaterAutoOn ? true : heaterAutoOff ? false : manualHeaterOn
  ...
  const handleFanCommand = async (turnOn: boolean) => {
    ...
    setTogglingFan(true)
    try {
      const res = await fetch("/api/fan/toggle", {
        method: "POST",
  ...
  const cards = [
    {
      ...
      unit: "°C",
      icon: Thermometer,
      color: temp > 34 ? "text-red-500" : temp < 30 ? "text-blue-500" : "text-green-500",
    ...
    },
    {
      ...
      unit: "PPM",
      icon: Wind,
      color: latest && latest.air_quality > 400 ? "text-orange-500" : "text-green-500",
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Controls Dashboard</h1>
          <p className="text-sm text-gray-500 mt-1">
            {time.toLocaleDateString()} {time.toLocaleTimeString()} &middot; {readings.length} readings today
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {cards.map((card) => {
          ...
          return (
            <div key={card.label} className={`rounded-xl ${card.bg} p-6 border border-gray-200 shadow-sm`}>
              ...
              <p className={`text-3xl font-bold ${card.color}`}>
                {card.value}
                <span className="text-lg font-normal text-gray-400 ml-1">{card.unit}</span>
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={`rounded-xl border p-6 shadow-sm ${
          effectiveFan ? "bg-green-50 border-green-200" : "bg-white border-gray-200"
        }`}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-medium text-gray-600">Fan Control</span>
            <Fan className={effectiveFan ? "text-green-500" : "text-gray-400"} size={28} />
          </div>

          <div className="flex items-center gap-3 mb-3">
            <span className={`text-2xl font-bold ${effectiveFan ? "text-green-600" : "text-gray-500"}`}>
              {effectiveFan ? "RUNNING" : "OFF"}
            </span>
            {fanAutoOn && (
              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">AUTO</span>
            )}
          </div>
          ...
          {fanAutoOn && (
            <p className="text-xs text-red-500 font-medium">
              Auto-override active — manual control locked while temperature exceeds 34°C
            </p>
          )}
        </div>
        ...
        {/* ────────────── HEATER CARD ────────────── */}
        <div className={`rounded-xl border p-6 shadow-sm ${
          effectiveHeater ? "bg-red-50 border-red-200" : "bg-white border-gray-200"
        }`}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-medium text-gray-600">Heater Control</span>
            <Zap className={effectiveHeater ? "text-red-500" : "text-gray-400"} size={28} />
          </div>

          <div className="flex items-center gap-3 mb-3">
            <span className={`text-2xl font-bold ${effectiveHeater ? "text-red-600" : "text-gray-500"}`}>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            />
            <span className="text-sm font-medium text-gray-500">
              {heaterAutoOn ? "Auto ON" : heaterAutoOff ? "Auto OFF" : manualHeaterOn ? "ON" : "OFF"}
            </span>
          </div>
